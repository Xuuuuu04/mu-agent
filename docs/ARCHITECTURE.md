...
研究状态使用版本化 JSON envelope 并原子替换。损坏文件 fail-closed，避免在读失败后用空状态覆盖真实历史。行情 watchdog 同时保留 cooldown 状态和独立健康快照；生产阈值告警要求 iFind 与腾讯两个独立来源各自携带交易所时间、仍在 freshness 窗口且价格在 20bps 内形成 quorum，验证源缺失、陈旧或冲突只留降级证据、不触发可疑价格告警。公告 monitor 以巨潮官方直连为主源，请求失败才切到 iFind，官方空结果不误触发兜底；07:00-23:30 每 15 分钟检查持仓近 24 小时公告，静默窗口不调用外部源，全程不调用 LLM。重大事件必须先成功进入可靠投递链再标记 seen，投递失败会在下一轮重试。组合风险只接受足够新、数量有效且覆盖完整的价格。
